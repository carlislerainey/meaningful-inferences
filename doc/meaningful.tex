...
Each of these questions require increasing levels of analysis and substantive interpretation and each is receiving decreasing levels of attention in political science research. Virtually every research article in empirical science makes an argument about the direction of the effect using a null hypothesis significance test. Almost all of these articles report some measure of effect size, even if it is just in the form of a table of regression coefficients. 
...
As an alternative, we suggest the explicit-test approach. To explicitly test a hypothesis of a meaningful effect, researchers simply use their substantive judgement to partition the set of possible effects into meaningful effects and trivial effects. They then conduct a formal hypothesis test that the true effect is equivalent to the smallest substantively meaningful effect. This approach addresses the flaws of both the sign-and-significance and the magnitude-and-significance approaches.
...
The magnitude-and-significance approach also treats Studies C and E similarly because both estimates are ``not statistically significant.'' However, Study C is consistent with both large, meaningful effects and small, negligible effects. Study E on the other hand, is only consistent with small, negligible effects.
...
% Overview
In order to make a compelling argument that an effect is substantively meaningful, the researcher must carefully define exactly which effects are and are not substantively meaningful. To do this, researchers simply choose an $m$ such that effects smaller than $m$ are considered substantively negligible and effects larger than $m$ are considered substantively meaningful. This $m$ partitions the possible effects into those that are substantively meaningful and those that are substantively negligible.
...
\carlisle{[ctk - I don't think that we are making the claim that $m$ hypothesis testing will cut down on people running 18 models and picking the best three to report, but maybe this reads differently to you here?]}
Formal hypothesis tests and judgments about substantive importance are qualitatively different decisions and have different strengths and weaknesses. Estimation and hypothesis tests are relatively automatic and ``objective,'' but not transparent. Researchers do not fit one model and report the single $p$-value. Instead they fit many models and report the one that ``makes most sense'' in light of their approach, theoretical model, normative concerns, and the results of the model. (\citealt{GerberMalhotra2008}; \citealt{SimmonsNelsonSimonsohn2011}; \citealt{Francis2013}; \citealt{SimmonsNelsonSimonsohn2014}; and \citealt{EsareyWu2014}; see also \citealt{GelmanLoken2014}). Substantive judgments about effect sizes, however, require a large initial investment of careful thought and this judgment demands subjectivity (see \citealt{Rainey2014} for more discussion on how to do this). However, this subjective judgment is transparent, readers are free to reject the author's judgment and substitute their own. Further, ``automatic'' and ``objective'' procedures are not always (or perhaps usually) desirable.  Indeed, \cite{Kirk1996} writes:
...
\carlisle{[ctk - Kevin says "arguing for $m$ post model results might require a little more defense/reasoning." We talked about how we should highlight this as a benefit of using our approach because it requires researchers to defend their results and choices more carefully and to elaborate on their interpretation.]}
...
\carlisle{[ctk - Kevin thinks this figure shows the importance of confidence intervals but is not illustrative of testing against an $m$.]}

\section*{Replication of Kam and Zechmeister (2013)}

% overview of kz
\carlisle{[ctk - we want to think in detail of how to discuss the size of the effects here. In this case, even tiny-ish effects are perhaps meaningful. It is probably worth engaging in what's meaningful in this situation.]}
...
In the case of lab experiments, the researcher should think carefully about how the study design maps onto the key real-world concepts. For example, is their reason that a negative ad shown in the laboratory has an effect similar in magnitude as an ad viewed at home after dinner? It seems plausible that the effect occurs in the same \emph{direction}, but the \emph{magnitudes} might be quite different.
...
\carlisle{[ctk - Kevin is worried that ``taking the CIs into account, people would switch to smaller N studies and then use the winder confidence intervals to say that the extreme interval endpoints give their findings meaning," but we say no, it would actually be the opposite, we care more about the lower bound. These papers are overestimating their results. How can we highlight this point?]}
